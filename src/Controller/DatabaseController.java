...
public class DatabaseController {
	 ...
	 public static ResultSet DBConnect(String table){
		 try{
			 ...
			 try{
			conn = DriverManager.getConnection("jdbc:mysql://localhost:3306/avansconcert", "root", "");
			 }
			 catch(Exception e){
			...
			if(table != null){
				...
				rs = st.executeQuery("SELECT * FROM planning"
					+ " LEFT JOIN artists ON planning.artist_id = artists.id"
	 ...
	 public static void EditArtist(int id, String artist, String description){
		 try{
			 	Connection conn = DriverManager.getConnection("jdbc:mysql://localhost:3306/avansconcert", "root", "");
				Statement st = conn.createStatement();
				String sql = "UPDATE artists SET artist='"+artist+"', description='"+description+"' WHERE id = "+id;
				st.executeUpdate(sql);
				st.close();
		 }
		 catch(Exception e){
